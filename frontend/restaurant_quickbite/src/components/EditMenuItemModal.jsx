...
const EditMenuItemModal = ({ item, onClose, onUpdated }) => {
  ...
  const [form, setForm] = useState({
    ...
    addOns: item.addOns || [],
    nutritionalInfo: item.nutritionalInfo || { calories: "", protein: "", carbs: "", fats: "" },
  });
  ...
  const handleSubmit = async (e) => {
    ...
    setLoading(true);
    try {
      // Find selected category image if changed
      const selectedCat = adminCategories.find(c => c.name === form.category);
      const categoryImage = selectedCat ? selectedCat.image : null;

      const data = { ...form, categoryImage };
      const res = await updateMenuItem(item._id, data, token);
